...
class Key:
    ...
    def encrypt(self, plain: str) -> str:
        ...
        while counter < len(plain):
            for i in range(self.size):
                s[i] = get_code(plain[counter] if counter <
                                len(plain) else self.__filler)
                counter += 1
            for i in range(self.size):
                code = 0
    ...
    def determinant(mat: list[list[int]]) -> int:
        ...
        for i in range(len(mat)):
            det += (1 if i % 2 == 0 else -1) * \
                mat[0][i] * Key.determinant(Key.minor(mat, 0, i))
        return det
    ...
    def adjacent(mat: list[list[int]]) -> list[list[int]]:
        ...
        for i in range(size):
            for j in range(size):
                sign = -2 * ((i+j) % 2) + 1
                adj[j][i] = (1 if (i+j) % 2 == 0 else -1) * \
                    Key.determinant(Key.minor(mat, i, j))
        return adj
    ...
    def decrypt(self, cipher: str) -> str:
        ...
        while counter < len(cipher):
            for i in range(self.size):
                s[i] = get_code(cipher[counter] if counter <
                                len(cipher) else self.__filler)
                counter += 1
            for i in range(self.size):
                code = 0
                for j in range(self.size):
