...
class Tableau:
    ...
    def __init__(self, size: int):
        self.__table = [[(i+j) % size for i in range(size)]
                        for j in range(size)]
    ...
    def encrypt(self, plain: str, key: str) -> str:
        ...
        while counter < len(plain):
            code = self[get_code(plain[counter]),
                        get_code(key[counter % len(key)])]
    ...
    def decrypt(self, ciph: str, key: str) -> str:
        ...
        while counter < len(ciph):
            code = self.index_in_row(
                ciph[counter], get_code(key[counter % len(key)]))
            plain += get_char(code)
            counter += 1
        return plain
